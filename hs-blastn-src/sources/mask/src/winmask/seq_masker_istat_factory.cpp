...
#include "seq_masker_istat_bin.hpp"
                                                  ...
                                                  bool use_ba )
{
    //try
    {
        ...
        switch( stat_type ) {
			...
			default: 
			{
				//NCBI_THROW(Exception, eCreateFail, "unrecognized unit counts format");
				std::string err = "unrecoginized unit counts format.";
				error_and_exit(err);
			}
        }
	...
	std::string err = "could not create a unit counts container.";
	error_and_exit(err);
}
