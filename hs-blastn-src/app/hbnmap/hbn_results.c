    ...
    kstring_t* out)
{
    ...
    kputc(tab, out);
    ksprintf(out, "AS:i:%d", hsp->score); ///  dp score
    if (dump_cigar) {
        kputc(tab, out);
    ...
    kstring_t* out)
{
    ...
    /// qs
    kputc(tab, out);
    ksprintf(out, "qs:i:%d", hsp->query.offset);
    /// qe
    kputc(tab, out);
    ...
    /// evalue
    kputc(tab, out);
    ksprintf(out, "EV:f:%g", hsp->evalue);
    /// bit score
    kputc(tab, out);
    ksprintf(out, "BS:f:%g", hsp->bit_score);
    /// identity
    kputc(tab, out);
    ksprintf(out, "PI:f:%g", hsp->hsp_info.perc_identity);
    kputc('\n', out);
}
