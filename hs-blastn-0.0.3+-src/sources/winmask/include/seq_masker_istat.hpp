 ...
 **/
class NCBI_XALGOWINMASK_EXPORT CSeqMaskerIstat 
{
public:
    ...
    explicit CSeqMaskerIstat() : opt_data_(0,0) { total_ = 0; }
...
public:
    void DumpCSeqMaskerIstat(FILE* file)
    {
        ASSERT(file != NULL);
        int n;
        
        n = fwrite(&total_, sizeof(Uint8), 1, file);
        ASSERT(n == 1);        
        
        n = fwrite(&threshold, sizeof(Uint4), 1, file);
        ASSERT(n == 1);
        
        n = fwrite(&textend, sizeof(Uint4), 1, file);
        ASSERT(n == 1);        
        
        n = fwrite(&max_count, sizeof(Uint4), 1, file);
        ASSERT(n == 1);      
        
        n = fwrite(&use_max_count, sizeof(Uint4), 1, file);
        ASSERT(n == 1);        
        
        n = fwrite(&min_count, sizeof(Uint4), 1, file);
        ASSERT(n == 1);   
        
        n = fwrite(&use_min_count, sizeof(Uint4), 1, file);
        ASSERT(n == 1);        
        ...
        n = fwrite(&ambig_unit, sizeof(CSeqMaskerWindow::TUnit), 1, file);
        ASSERT(n == 1);        
    }
    
    void LoadCSeqMaskerIstat(FILE* file)
    {
        ASSERT(file != NULL);
        int n;
        
        n = fread(&total_, sizeof(Uint8), 1, file);
        ASSERT(n == 1);        
        
        n = fread(&threshold, sizeof(Uint4), 1, file);
        ASSERT(n == 1);
        
        n = fread(&textend, sizeof(Uint4), 1, file);
        ASSERT(n == 1);        
        
        n = fread(&max_count, sizeof(Uint4), 1, file);
        ASSERT(n == 1);      
        
        n = fread(&use_max_count, sizeof(Uint4), 1, file);
        ASSERT(n == 1);        
        
        n = fread(&min_count, sizeof(Uint4), 1, file);
        ASSERT(n == 1);   
        
        n = fread(&use_min_count, sizeof(Uint4), 1, file);
        ASSERT(n == 1);        
        ...
        n = fread(&ambig_unit, sizeof(CSeqMaskerWindow::TUnit), 1, file);
        ASSERT(n == 1);        
    }    
    ...
    void Print()
    {
        ...
        cout << "use_max_count = " << use_max_count << endl;
        cout << "min_count = " << min_count << endl;
        cout << "use_min_count = " << use_min_count << endl;
        cout << "unit_size = " << (Uint4)unit_size << endl;
        cout << "ambig_unit = " << ambig_unit << endl;
    }
